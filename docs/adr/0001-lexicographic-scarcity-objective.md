...
Extraction is modelled as an ordinary input-less Recipe with a machine cost, not as a boundary condition. Without that, Resources whose weight is effectively zero — water above all, since extractors have no node limit — would be free in both phases, and nothing at all would stand between the optimiser and an absurd volume of them. Counting extractors in phase 2 prices them as buildings instead of inventing a weight for them.

Be precise about how far that goes, because it is easy to over-claim. The machine cost decides only where Scarcity Cost ties. Given two Recipes that spend the same ore, the one drinking 10,000 m³/min of water loses, because 84 Water Extractors are 84 machines. But given a Recipe that saves even a single unit of ore by drinking that same water, it wins — the lexicographic order is strict, and no number of machines outranks any scarcity saving whatsoever. Both behaviours are tested. Trading the two off against each other is exactly the blended objective this ADR rejects, so the limit is the price of the decision rather than a defect in it.
