...
The Resource Weight table is a second data source, separate from `Docs.json`, which carries no map data. It is computed from node counts × purity multiplier × miner rate rather than copied from a community source: published totals disagree with each other, and at least one contradicts the node distribution printed on the same page. Baseline is Miner Mk.1 at 100%. Between mined ores the tier scales uniformly so relative weights hold, but the ratio between solid ores and fluids does shift with tier. That imprecision is accepted, not hidden.